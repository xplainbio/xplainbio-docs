...
Поле xMD работает аналогично полям HGMD и ClinVar, но использует доступные пользователю локальные базы данных мутаций в качестве источника, даже если они не подлкючены к текущему проекту.